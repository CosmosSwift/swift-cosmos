...
protocol TransactionVerifiableBySignature: Transaction {
    var signatures: [Data] { get }
    var signers: [AccountAddress] { get }
    var pubKeys: [PublicKeyProtocol] { get } // If signer already has pubkey in context, this list will have nil in its place
    func signBytes(for request: Request, using account: AccountProtocol)-> Data
}
