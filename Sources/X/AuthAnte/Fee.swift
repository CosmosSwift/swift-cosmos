...
protocol TransactionWithFee: TransactionWithGas {
    var fees: [Coin] { get }
    var payer: AccountAddress { get }
}
...
struct MempoolFeeDecorator: AnteDecorator {
    func anteHandle(
        request: Request,
        ...
        guard let transaction = transaction as? TransactionWithFee else {
            throw CosmosError.wrap(error: CosmosError.invalidTransaction, description: "Tx must be a FeeTx")
        }
 /*
         feeTx, ok := tx.(FeeTx)
         if !ok {
...
struct DeductFeeDecorator: AnteDecorator {
    
    let accountKeeper: AccountKeeper
    let supplyKeeper: SupplyKeeper
    
    
    init(_ accountKeeper: AccountKeeper, _ supplyKeeper: SupplyKeeper) {
